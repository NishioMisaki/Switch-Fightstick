...
static uint8_t HatchEgg2(USB_JoystickReport_Input_t* const ReportData, uint16_t count)
{
	if (count % 5 == 0)
		ReportData->Button |= SWITCH_A;
...
void HatchEggs_Module2(USB_JoystickReport_Input_t* const ReportData)
{
	switch (state) {
	case RESET_POSITION_1:
		...
		break;
	case HATCH_EGG:
		if (!HatchEgg2(ReportData, duration_count))
			break;
